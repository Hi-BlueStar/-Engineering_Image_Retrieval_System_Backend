...
- 將指定的小組件合併回其對應的大組件。
- 儲存：原始影像、已清理/合且位置與原始影像相同的影像，
  以及一個合併後的大組件在畫布中隨機重新排列且不重疊的版本。
"""
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if rss_delta is not None:
                    parts.append(
                        f"ΔRSS={_fmt_bytes(rss_delta)} \
                            (from {_fmt_bytes(rss_before)} \
                                to {_fmt_bytes(rss_after)})"
                    )
                else:
...
def auto_binarize(
    ...
    - 若未指定閾值，使用 Otsu 自適應；
      同時計算黑/白底版本，選取前景像素數較少的一種。
    """
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    if bin_thresh and 0 < bin_thresh < 255:
        _, bin_white = cv2.threshold(
            gray,
            bin_thresh,
            255,
            cv2.THRESH_BINARY
    ...
    total = h * w
    candidates = [
        (cnt_w, "black_bg", bin_white),
        (cnt_b, "white_bg", bin_black)
    ]
    candidates = [
        (c, tag, b)
        for (c, tag, b) in candidates
        if 0 < c < total * 0.95
    ]
...
# ------------------------------------------------------------
# 推論前處理：將 OpenCV 影像轉換為 Torch Tensor
# ------------------------------------------------------------
def preprocess_for_inference(
    image_payload: np.ndarray,
    img_size: int = 512
) -> torch.Tensor:
    """
    將讀取的 BGR 影像轉換為模型輸入張量。
    1. BGR -> RGB
    2. Resize & Padding to square
    3. ToTensor (Normalize to [0, 1])
    4. ImageNet Mean/Std Normalization
    """
    # 1. 轉換為 RGB PIL Image
    image_rgb = cv2.cvtColor(image_payload, cv2.COLOR_BGR2RGB)
    ...
    return inference_transform(pil_image)
...
class Component:
    ...
    roi_mask: np.ndarray  # 優化：僅儲存裁切後的 ROI 遮罩，而非全圖大小
...
def analyze_components(bw01: np.ndarray) -> list[Component]:
    ...
    """
    # 使用 WithStats 直接取得統計資訊，避免後續
    # 重複計算 boundingRect 與 countNonZero
    ...
    for lbl in range(1, num_labels):
        ...
        # 這裡依舊使用 stats 裡的 area (像素面積)，
        #     若需維持原邏輯(BBox面積)則自行計算
        bbox_area = w * h
...
def filled_region_from_component(comp: Component) -> np.ndarray:
    """
    由單一大元件的 0/1 mask，取 external contour 後填滿，
        得到其「外輪廓內部」區域。
    回傳：uint8 0/1，大小等同原圖。
...
def assign_small_to_large(
    ...
    for s in small:
        ...
        for L in large:
            ...
            # 安全檢查：確保座標在大元件 ROI 範圍內
            #          (理論上 BBox check 已過濾，但防呆)
            valid_idx = (
                (rel_y >= 0) & (rel_y < Lh) & (rel_x >= 0) & (rel_x < Lw)
            )
...
def merge_small_into_large(
    ...
    """
    Union each large component mask with the masks of the
    small components assigned to it.
    """
...
def compose_on_original_positions(
    ...
    """
    Keep only merged large components (large + assigned small)
...
def random_arrange_components(
    ...
    *,
    ...
    """
    Randomly place each merged large component (with its small components)
        on the canvas without overlap.
    ...
    for comp in comps_data:
        ...
        if chosen is None:
            ...
            if (
                ...
                and not any(
                    _boxes_overlap(candidate, p, padding) for p in placed
                )
            ):
                chosen = candidate
            else:
...
def run_pipeline(
    ...
    - Step 3: 將指派的小元件貼回大元件，輸出：
        ...
        * 大元件貼回後隨機排列且不重疊的全圖
        * 每個「處理後大元件」的獨立輸出圖
            （裁切為外框+padding，存放於 large_components/）
    ...
    Args:
        ...
        padding: 隨機排列時盒子之間預留的間距（像素），
            同時也作為大元件裁切輸出的外框 padding
    ...
    if src is None:
        raise ValueError(
            f"cv2.imread/imdecode 讀取失敗：{input_path.resolve()}"
        )
    ...
    print(
        f"[OK] 已儲存隨機排列 {len(paths['random'])} 張；\
            範例：{paths['random'][0] if paths['random'] else '無'}"
    )
    print(
        f"[OK] 已輸出處理後大元件於資料夾：{paths['large_dir']} \
            ({len(saved_large_paths)} files)"
    )
...
if __name__ == "__main__":
    ...
    # --- 範例 1：完整輸出模式 ---
    demo_path = Path(
        "data/engineering_images_100dpi/固定夾塊/175H10-DSV-000-10200.png"
    )
