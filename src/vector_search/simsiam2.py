...
# GeometryAwareTransform and make_inference_transform have been
# moved to augmentations.py
...
class SimSiam(nn.Module):
    ...
    def __init__(
        ...
        if in_channels != 3:
            ...
            # 若載入預訓練權重，可將 RGB 權重平均化到單通道
            # (由使用者自行決定是否需要更複雜的遷移策略)
            if pretrained and in_channels == 1:
                with torch.no_grad():
                    # Sum over channel dimension (dim 1) and divide by 3
                    # shape: [64, 3, 7, 7] -> [64, 1, 7, 7]
                    new_conv.weight[:] = old_conv.weight.sum(
                        dim=1,
                        keepdim=True
                    ) / 3.0
